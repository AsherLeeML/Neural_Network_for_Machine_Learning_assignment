...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  class_num = size(data.inputs, 2);
  output_delta = (class_prob-data.targets);
  ret.hid_to_class = output_delta * hid_output'/class_num + wd_coefficient * model.hid_to_class;
  
  error_derivated = model.hid_to_class' * output_delta .* hid_output .* (1 - hid_output);
  ret.input_to_hid = error_derivated * data.inputs'/class_num + model.input_to_hid * wd_coefficient;
end
